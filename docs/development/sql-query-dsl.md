...
Every tuple in `vals` must carry exactly one value per field. The width is
checked against the field list — not just against the other tuples — and a
mismatch panics before anything is written, rather than rendering a query that is
wrong rather than merely invalid: a single over-wide tuple would take the
equality shortcut and drop every value past the first, and an under-wide tuple
would compare a row value against a field list of a different arity. A row of
width zero is rejected on the same grounds: the writers emit a placeholder for
the first cell of every row unconditionally, so it would leave a `$n` in the
query with no parameter bound behind it.
