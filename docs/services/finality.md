...
(module `github.com/LFDT-Panurus/panurus`, pinned to `github.com/hyperledger-labs/fabric-smart-client
v0.18.0` in `go.mod`). This document mirrors the structure of that upstream project's
...
```mermaid
stateDiagram-v2
    [*] --> Pending: Append<br/>debtor & every creditor,<br/>each on its own Append call
