...
> **Performance Note**: The `simple` driver pushes the token limit down into the SQL
> query (`LIMIT ?`) to reduce I/O overhead. The default `sherdlock` driver does **not**:
> its `maxTokensPerSelection` is an in-process CPU/lock-attempt bound only, not a
> DB-query or memory bound (see [Token Iteration Depth Limit](#1-token-iteration-depth-limit)).
...
**How it is enforced (driver-dependent)**:

- **`simple` driver**: enforced at **two levels**:
  1. **Database query level**: `UnspentTokensIteratorBy` passes `maxTokensPerSelection`
     into the query so SQL includes a `LIMIT ?` clause, preventing excess rows from being
     fetched (`simple/selector.go`).
  2. **Application level**: an iterator counter provides a defense-in-depth safety check.
- **`sherdlock` driver (default)**: enforced at the **application level only**. All three
  sherdlock fetchers ignore the limit parameter, and the cached fetcher refreshes by
  scanning the full spendable-token set (`SpendableTokensIteratorBy(ctx, "", "")`); the
  `maxTokensPerSelection` ceiling is applied as an in-process iteration counter. It bounds
  CPU and lock-acquisition attempts per selection, **not** the number of rows the database
  returns or the memory the fetcher holds (`sherdlock/fetcher.go`).

**Performance impact**: the DB-level `LIMIT` (`simple` only) significantly reduces I/O
overhead:
- Without `LIMIT`: the database may scan many rows, returning them one-by-one.
- With `LIMIT`: the database stops after returning the configured number of rows.
- Result: markedly faster query execution for large token sets.

Under `sherdlock` there is no such query-level reduction; the limit still protects against
unbounded CPU and lock-attempt work, but the database query and the fetcher's working set
are unbounded by `maxTokensPerSelection`.
...
> **Lowering this alone is safe.** `maxLocksPerTransaction` must stay
> `≤ maxTokensPerSelection` (see [Validation Rules](#validation-rules)). When you
> set only `maxTokensPerSelection` and leave `maxLocksPerTransaction` at its
> default, an unset `maxLocksPerTransaction` is resolved to the *smaller* of its
> own default and your tightened `maxTokensPerSelection`, so the invariant still
> holds and the node starts normally. Only an **explicitly configured**
> `maxLocksPerTransaction` that exceeds `maxTokensPerSelection` is rejected at
> startup — that is an operator mistake, not a defaulting gap.
...
**Error message**: `"token selection aborted: exceeded max retry cycles (3) after examining X tokens and Y lock attempts"`
(the simple selector reports `exceeded max retries (3) ...` and wraps the typed reason
...
> **Migration note**: the deprecated `numRetries` key is superseded by
> `limits.maxRetries`. The default is **3** cycles, unchanged from the pre-limits
> behaviour, so a deployment that never set either key keeps its previous
> worst-case selection latency. Because the wall-clock timeout defaults to
> `30s + maxRetries * retryInterval` (see [Wall-Clock Timeout](#5-wall-clock-timeout)),
> the default worst case is ~45s with the default 5s `retryInterval`. Raising
> `maxRetries` raises that worst case proportionally.
...
The count is released when the transaction's locks are released
(`UnlockByTxID`) or when its selector is closed, and stale counters are
reclaimed once a transaction has been idle longer than `leaseExpiry`. Periodic
lock cleanup does **not** reset live counters, so the ceiling cannot be
circumvented by waiting for a cleanup tick.

> **Scope of the counter — it is a per-selection safety ceiling, not a hard
> per-transaction cap.** Despite the name, the count is scoped to a single
> selection, not to the whole transaction:
> - A transaction that runs **several transfer actions** takes one selector per
>   action, and each action's selector is closed independently. Closing a
>   selector drops its counter, so every action starts from a fresh budget: a
>   request with N actions may hold up to `N × maxLocksPerTransaction` locks.
> - The `simple` in-memory locker keys the counter inside the **per-owner
>   shard**, so a single selection that spends from two wallets is counted
>   separately in each shard and may hold up to `2 × maxLocksPerTransaction`.
>
> The limit is still an effective bound on the CPU and lock-store growth of any
> *single* selection — its purpose here — but it is not a global per-transaction
> guarantee. If you need a hard ceiling across a multi-action or multi-wallet
> transaction, set `maxLocksPerTransaction` to the per-action budget you can
> tolerate multiplied down accordingly.
...
`30s + maxRetries * retryInterval`. With the default 3 retry cycles and a
5s `retryInterval` the effective default is 45s.
...
- Keep it above `maxRetries * retryInterval`, or the timeout will fire
...
token:
  selector:
    limits:
      ...
      maxRetries: 10                    # Reasonable for transient issues
...
token:
  selector:
    limits:
      ...
      maxRetries: 20                    # More retries for high contention
...
token:
  selector:
    limits:
      ...
      maxRetries: 5                     # Fewer retries for faster failure
...
bound the *rate* at which a wallet may issue selections. For that, Panurus offers:
...
- The built-in limiter is per process. If a wallet's traffic is spread over several nodes,
  ...
  cluster-wide budget.
- Because a policy implemented in your own `Locker` lives entirely in your code, its scope
  (per process vs shared across a cluster), persistence, and lifecycle are under your
  control.
