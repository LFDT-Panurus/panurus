...
> `limits.maxRetries`. A deployment that never set either now uses the default of
> **10** cycles (previously 3). Because the wall-clock timeout defaults to
> `30s + maxRetries * retryInterval` (see [Wall-Clock Timeout](#5-wall-clock-timeout)),
> this raises the worst-case selection latency from ~15s to ~80s with the default
> 5s `retryInterval`. Set `limits.maxRetries` explicitly to keep the old behaviour.
...
`30s + maxRetries * retryInterval`. With the default 10 retry cycles and a
