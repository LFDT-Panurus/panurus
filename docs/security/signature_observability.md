...
Because identities are attacker-supplied, `idleTTL` alone would bound memory only by
time: a flood of distinct identities could allocate per-principal state and a token
bucket for each, all retained until the TTL elapses. `maxPrincipals` therefore caps the
number of tracked principals *and* the number of token buckets held at once by count.
...
```go
verifier, err := signatureService.OwnerVerifier(ctx, id)
...
| `maxPrincipals` | `10000` | Upper bound, by count, on both the tracked principals and the token buckets held at once, so a flood of distinct identities cannot grow the throttle's memory without limit. At the cap the oldest idle `normal` entry is evicted; if only throttled entries remain, a new identity is treated as `normal` rather than displacing one. A negative value disables the cap (not recommended in production). |
