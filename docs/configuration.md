...
- **maxPrincipals**: Maximum number of per-principal entries (and token buckets) held at once, bounding the policy's memory footprint under a flood of distinct attacker-supplied identities. When the cap is reached and a new principal arrives, the oldest idle `normal` entry is evicted (or the oldest `soft` entry when no `normal` one exists); `blocked` entries are never evicted. A negative value disables the cap (not recommended in production)
