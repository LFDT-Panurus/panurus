...
func (d *Deserializer) getVerifier(ctx context.Context, vd driver.VerifierDeserializer, id driver.Identity, op sigobserve.Op, role sigobserve.Role) (driver.Verifier, error) {
	// Fast path for an un-observed deserializer (the default, and the one every validator uses):
	// skip the id.UniqueID() hash and the two time.Now() calls that Start/Done would make, and
	// hand back the verifier unwrapped. With a Nop observer that work is computed only to be
	// discarded, and this runs once per verifier resolution on the validation hot path.
	if d.observer == sigobserve.Nop {
		return vd.DeserializeVerifier(ctx, id)
	}
