...
// buildTransferInputsMetadata builds the per-input transfer metadata (sender identity + audit
// info) for each loaded input token.
func (s *TransferService) buildTransferInputsMetadata(ctx context.Context, ws driver.WalletService, inputTokens []*token.Token, ids []*token.ID) ([]*driver.TransferInputMetadata, error) {
	...
	for i, t := range inputTokens {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed getting audit info for sender identity [%s]", driver.Identity(t.Owner).String())
		}
...
// buildTransferOutputMetadata builds the transfer metadata (audit info + receivers) for a single
// output, handling the redeem (no owner) case separately from the normal recipient-lookup case.
func (s *TransferService) buildTransferOutputMetadata(ctx context.Context, ws driver.WalletService, output *actions.Output, outputMetadataRaw []byte) (*driver.TransferOutputMetadata, error) {
	...
	if len(output.Owner) == 0 { // redeem
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed getting audit info for sender identity [%s]", driver.Identity(output.Owner).String())
		}
...
func (s *TransferService) Transfer(
	...
	*driver.TransferMetadata,
	...
	// inputs
	transferInputsMetadata, err := s.buildTransferInputsMetadata(ctx, ws, inputTokens, ids)
	if err != nil {
		return nil, nil, err
	}
	...
	for _, output := range outs {
		om, err := s.buildTransferOutputMetadata(ctx, ws, output, outputMetadataRaw)
		if err != nil {
			return nil, nil, err
		}
