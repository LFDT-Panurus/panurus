// Code generated by counterfeiter. DO NOT EDIT.
package mock

import (
	"sync"

	"github.com/LFDT-Panurus/panurus/token/driver"
)

type TokenManagerServiceProvider struct {
	ConfigurationForStub        func(string, string, string) (driver.Configuration, error)
	configurationForMutex       sync.RWMutex
	configurationForArgsForCall []struct {
		arg1 string
		arg2 string
		arg3 string
	}
	configurationForReturns struct {
		result1 driver.Configuration
		result2 error
	}
	configurationForReturnsOnCall map[int]struct {
		result1 driver.Configuration
		result2 error
	}
	GetTokenManagerServiceStub        func(driver.ServiceOptions) (driver.TokenManagerService, error)
	getTokenManagerServiceMutex       sync.RWMutex
	getTokenManagerServiceArgsForCall []struct {
		arg1 driver.ServiceOptions
	}
	getTokenManagerServiceReturns struct {
		result1 driver.TokenManagerService
		result2 error
	}
	getTokenManagerServiceReturnsOnCall map[int]struct {
		result1 driver.TokenManagerService
		result2 error
	}
	UpdateStub        func(driver.ServiceOptions) error
	updateMutex       sync.RWMutex
	updateArgsForCall []struct {
		arg1 driver.ServiceOptions
	}
	updateReturns struct {
		result1 error
	}
	updateReturnsOnCall map[int]struct {
		result1 error
	}
	invocations      map[string][][]interface{}
	invocationsMutex sync.RWMutex
}

func (fake *TokenManagerServiceProvider) ConfigurationFor(arg1 string, arg2 string, arg3 string) (driver.Configuration, error) {
	fake.configurationForMutex.Lock()
	ret, specificReturn := fake.configurationForReturnsOnCall[len(fake.configurationForArgsForCall)]
	fake.configurationForArgsForCall = append(fake.configurationForArgsForCall, struct {
		arg1 string
		arg2 string
		arg3 string
	}{arg1, arg2, arg3})
	stub := fake.ConfigurationForStub
	fakeReturns := fake.configurationForReturns
	fake.recordInvocation("ConfigurationFor", []interface{}{arg1, arg2, arg3})
	fake.configurationForMutex.Unlock()
	if stub != nil {
		return stub(arg1, arg2, arg3)
	}
	if specificReturn {
		return ret.result1, ret.result2
	}
	return fakeReturns.result1, fakeReturns.result2
}

func (fake *TokenManagerServiceProvider) ConfigurationForCallCount() int {
	fake.configurationForMutex.RLock()
	defer fake.configurationForMutex.RUnlock()
	return len(fake.configurationForArgsForCall)
}

func (fake *TokenManagerServiceProvider) ConfigurationForCalls(stub func(string, string, string) (driver.Configuration, error)) {
	fake.configurationForMutex.Lock()
	defer fake.configurationForMutex.Unlock()
	fake.ConfigurationForStub = stub
}

func (fake *TokenManagerServiceProvider) ConfigurationForArgsForCall(i int) (string, string, string) {
	fake.configurationForMutex.RLock()
	defer fake.configurationForMutex.RUnlock()
	argsForCall := fake.configurationForArgsForCall[i]
	return argsForCall.arg1, argsForCall.arg2, argsForCall.arg3
}

func (fake *TokenManagerServiceProvider) ConfigurationForReturns(result1 driver.Configuration, result2 error) {
	fake.configurationForMutex.Lock()
	defer fake.configurationForMutex.Unlock()
	fake.ConfigurationForStub = nil
	fake.configurationForReturns = struct {
		result1 driver.Configuration
		result2 error
	}{result1, result2}
}

func (fake *TokenManagerServiceProvider) ConfigurationForReturnsOnCall(i int, result1 driver.Configuration, result2 error) {
	fake.configurationForMutex.Lock()
	defer fake.configurationForMutex.Unlock()
	fake.ConfigurationForStub = nil
	if fake.configurationForReturnsOnCall == nil {
		fake.configurationForReturnsOnCall = make(map[int]struct {
			result1 driver.Configuration
			result2 error
		})
	}
	fake.configurationForReturnsOnCall[i] = struct {
		result1 driver.Configuration
		result2 error
	}{result1, result2}
}

func (fake *TokenManagerServiceProvider) GetTokenManagerService(arg1 driver.ServiceOptions) (driver.TokenManagerService, error) {
	fake.getTokenManagerServiceMutex.Lock()
	ret, specificReturn := fake.getTokenManagerServiceReturnsOnCall[len(fake.getTokenManagerServiceArgsForCall)]
	fake.getTokenManagerServiceArgsForCall = append(fake.getTokenManagerServiceArgsForCall, struct {
		arg1 driver.ServiceOptions
	}{arg1})
	stub := fake.GetTokenManagerServiceStub
	fakeReturns := fake.getTokenManagerServiceReturns
	fake.recordInvocation("GetTokenManagerService", []interface{}{arg1})
	fake.getTokenManagerServiceMutex.Unlock()
	if stub != nil {
		return stub(arg1)
	}
	if specificReturn {
		return ret.result1, ret.result2
	}
	return fakeReturns.result1, fakeReturns.result2
}

func (fake *TokenManagerServiceProvider) GetTokenManagerServiceCallCount() int {
	fake.getTokenManagerServiceMutex.RLock()
	defer fake.getTokenManagerServiceMutex.RUnlock()
	return len(fake.getTokenManagerServiceArgsForCall)
}

func (fake *TokenManagerServiceProvider) GetTokenManagerServiceCalls(stub func(driver.ServiceOptions) (driver.TokenManagerService, error)) {
	fake.getTokenManagerServiceMutex.Lock()
	defer fake.getTokenManagerServiceMutex.Unlock()
	fake.GetTokenManagerServiceStub = stub
}

func (fake *TokenManagerServiceProvider) GetTokenManagerServiceArgsForCall(i int) driver.ServiceOptions {
	fake.getTokenManagerServiceMutex.RLock()
	defer fake.getTokenManagerServiceMutex.RUnlock()
	argsForCall := fake.getTokenManagerServiceArgsForCall[i]
	return argsForCall.arg1
}

func (fake *TokenManagerServiceProvider) GetTokenManagerServiceReturns(result1 driver.TokenManagerService, result2 error) {
	fake.getTokenManagerServiceMutex.Lock()
	defer fake.getTokenManagerServiceMutex.Unlock()
	fake.GetTokenManagerServiceStub = nil
	fake.getTokenManagerServiceReturns = struct {
		result1 driver.TokenManagerService
		result2 error
	}{result1, result2}
}

func (fake *TokenManagerServiceProvider) GetTokenManagerServiceReturnsOnCall(i int, result1 driver.TokenManagerService, result2 error) {
	fake.getTokenManagerServiceMutex.Lock()
	defer fake.getTokenManagerServiceMutex.Unlock()
	fake.GetTokenManagerServiceStub = nil
	if fake.getTokenManagerServiceReturnsOnCall == nil {
		fake.getTokenManagerServiceReturnsOnCall = make(map[int]struct {
			result1 driver.TokenManagerService
			result2 error
		})
	}
	fake.getTokenManagerServiceReturnsOnCall[i] = struct {
		result1 driver.TokenManagerService
		result2 error
	}{result1, result2}
}

func (fake *TokenManagerServiceProvider) Update(arg1 driver.ServiceOptions) error {
	fake.updateMutex.Lock()
	ret, specificReturn := fake.updateReturnsOnCall[len(fake.updateArgsForCall)]
	fake.updateArgsForCall = append(fake.updateArgsForCall, struct {
		arg1 driver.ServiceOptions
	}{arg1})
	stub := fake.UpdateStub
	fakeReturns := fake.updateReturns
	fake.recordInvocation("Update", []interface{}{arg1})
	fake.updateMutex.Unlock()
	if stub != nil {
		return stub(arg1)
	}
	if specificReturn {
		return ret.result1
	}
	return fakeReturns.result1
}

func (fake *TokenManagerServiceProvider) UpdateCallCount() int {
	fake.updateMutex.RLock()
	defer fake.updateMutex.RUnlock()
	return len(fake.updateArgsForCall)
}

func (fake *TokenManagerServiceProvider) UpdateCalls(stub func(driver.ServiceOptions) error) {
	fake.updateMutex.Lock()
	defer fake.updateMutex.Unlock()
	fake.UpdateStub = stub
}

func (fake *TokenManagerServiceProvider) UpdateArgsForCall(i int) driver.ServiceOptions {
	fake.updateMutex.RLock()
	defer fake.updateMutex.RUnlock()
	argsForCall := fake.updateArgsForCall[i]
	return argsForCall.arg1
}

func (fake *TokenManagerServiceProvider) UpdateReturns(result1 error) {
	fake.updateMutex.Lock()
	defer fake.updateMutex.Unlock()
	fake.UpdateStub = nil
	fake.updateReturns = struct {
		result1 error
	}{result1}
}

func (fake *TokenManagerServiceProvider) UpdateReturnsOnCall(i int, result1 error) {
	fake.updateMutex.Lock()
	defer fake.updateMutex.Unlock()
	fake.UpdateStub = nil
	if fake.updateReturnsOnCall == nil {
		fake.updateReturnsOnCall = make(map[int]struct {
			result1 error
		})
	}
	fake.updateReturnsOnCall[i] = struct {
		result1 error
	}{result1}
}

func (fake *TokenManagerServiceProvider) Invocations() map[string][][]interface{} {
	fake.invocationsMutex.RLock()
	defer fake.invocationsMutex.RUnlock()
	copiedInvocations := map[string][][]interface{}{}
	for key, value := range fake.invocations {
		copiedInvocations[key] = value
	}
	return copiedInvocations
}

func (fake *TokenManagerServiceProvider) recordInvocation(key string, args []interface{}) {
	fake.invocationsMutex.Lock()
	defer fake.invocationsMutex.Unlock()
	if fake.invocations == nil {
		fake.invocations = map[string][][]interface{}{}
	}
	if fake.invocations[key] == nil {
		fake.invocations[key] = [][]interface{}{}
	}
	fake.invocations[key] = append(fake.invocations[key], args)
}

var _ driver.TokenManagerServiceProvider = new(TokenManagerServiceProvider)
