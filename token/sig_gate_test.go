...
func TestSignatureServiceLocalQuestionsAreNotGated(t *testing.T) {
	...
	g := newGatedService(gate)
	id := Identity("an_identity")
	g.provider.AreMeReturns([]string{id.UniqueID()}, nil)
	g.provider.IsMeReturns(true, nil)

	areMe, err := g.service.AreMe(t.Context(), id)
	require.NoError(t, err)
	assert.Equal(t, []string{id.UniqueID()}, areMe)
	isMe, err := g.service.IsMe(t.Context(), id)
	require.NoError(t, err)
	assert.True(t, isMe)
