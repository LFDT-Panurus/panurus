...
// overall receive timeout must be at least the sum of every wait it depends on:
// signature collection + audit + approval + slack. See issue #1266.
func TestReceiveTransactionTimeoutBudget(t *testing.T) {
	requiredMinimum := ttx.AnswerCollectionTimeout + ttx.AuditTimeout + fsc.ApprovalTimeout + requiredSlack

	require.GreaterOrEqual(t, ttx.DefaultReceiveTransactionTimeout, requiredMinimum,
		"responder receive timeout (%s) must be at least signature collection + audit + approval + slack (%s) — see issue #1266",
