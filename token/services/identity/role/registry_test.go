...
// --- per-wallet identity cap (see role.MaxIdentitiesPerWallet) ---

// countingStorage makes IdentityCount reflect the identities actually stored, the way a real
// backing store does, so the cap can be exercised at its boundary without performing
// role.MaxIdentitiesPerWallet bindings.
func countingStorage(storage *imock.WalletStoreService, alreadyStored int) {
	storage.IdentityCountStub = func(context.Context, idriver.WalletID, int) (int, error) {
		return alreadyStored + storage.StoreIdentityCallCount(), nil
	}
}

func TestBindIdentity_BelowLimitStores(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	countingStorage(storage, 0)

	require.NoError(t, reg.BindIdentity(t.Context(), []byte("id"), "e", "w", nil, "conf-1"))
	require.Equal(t, 1, storage.StoreIdentityCallCount())
}

func TestBindIdentity_AtLimitIsRejected(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	countingStorage(storage, role.MaxIdentitiesPerWallet)

	err := reg.BindIdentity(t.Context(), []byte("id"), "e", "w", nil, "conf-1")
	require.ErrorIs(t, err, role.ErrTooManyIdentities)
	require.Zero(t, storage.StoreIdentityCallCount(), "nothing must be written once the wallet is full")
}

func TestBindIdentity_LastSlotThenRejected(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	countingStorage(storage, role.MaxIdentitiesPerWallet-1)
	ctx := t.Context()

	require.NoError(t, reg.BindIdentity(ctx, []byte("id-1"), "e", "w", nil, "conf-1"))
	require.Equal(t, 1, storage.StoreIdentityCallCount())

	err := reg.BindIdentity(ctx, []byte("id-2"), "e", "w", nil, "conf-1")
	require.ErrorIs(t, err, role.ErrTooManyIdentities)
	require.Equal(t, 1, storage.StoreIdentityCallCount(), "the rejected binding must not be written")
}

// A full wallet still accepts an identity it already holds: re-binding it adds no row, so refusing
// would break idempotent re-registration without bounding anything.
func TestBindIdentity_AtLimitAllowsAlreadyBoundIdentity(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	countingStorage(storage, role.MaxIdentitiesPerWallet)
	storage.IdentityExistsReturns(true)
...
// Once a wallet is confirmed full it is sealed: further attempts must be refused without issuing
// another count, so repeated rejections cannot be turned into repeated database work.
func TestBindIdentity_SealedWalletDoesNotRequery(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	countingStorage(storage, role.MaxIdentitiesPerWallet)
	ctx := t.Context()

	require.ErrorIs(t, reg.BindIdentity(ctx, []byte("id-1"), "e", "w", nil, "conf-1"), role.ErrTooManyIdentities)
	countsAfterSealing := storage.IdentityCountCallCount()

	for i := range 5 {
		require.ErrorIs(t, reg.BindIdentity(ctx, []byte("id-2"), "e", "w", nil, "conf-1"), role.ErrTooManyIdentities, "attempt %d", i)
	}
	require.Equal(t, countsAfterSealing, storage.IdentityCountCallCount(), "a sealed wallet must not be counted again")
}

// The count is seeded once and maintained in memory, so the common path costs no query.
func TestBindIdentity_CountIsSeededOnce(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	countingStorage(storage, 0)
	ctx := t.Context()

	for range 5 {
		require.NoError(t, reg.BindIdentity(ctx, []byte("id"), "e", "w", nil, "conf-1"))
	}
	require.Equal(t, 1, storage.IdentityCountCallCount(), "expected a single seeding count")
	require.Equal(t, 5, storage.StoreIdentityCallCount())
}

// Wallets are accounted for independently: filling one must not affect another.
func TestBindIdentity_LimitIsPerWallet(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	storage.IdentityCountStub = func(_ context.Context, wID idriver.WalletID, _ int) (int, error) {
		if wID == "full" {
			return role.MaxIdentitiesPerWallet, nil
		}
	...
	require.ErrorIs(t, reg.BindIdentity(ctx, []byte("id"), "e", "full", nil, "conf-1"), role.ErrTooManyIdentities)
	require.NoError(t, reg.BindIdentity(ctx, []byte("id"), "e", "spacious", nil, "conf-1"))
}

// A failed write must give the reservation back, or transient storage errors would permanently
// consume slots.
func TestBindIdentity_FailedWriteReleasesReservation(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	countingStorage(storage, role.MaxIdentitiesPerWallet-1)
	storage.StoreIdentityReturns(errors.New("write failed"))
	ctx := t.Context()

	require.Error(t, reg.BindIdentity(ctx, []byte("id-1"), "e", "w", nil, "conf-1"))

	// The slot was released, so the retry is admitted rather than refused as over the limit.
	storage.StoreIdentityReturns(nil)
	require.NoError(t, reg.BindIdentity(ctx, []byte("id-1"), "e", "w", nil, "conf-1"))
}

func TestBindIdentity_CountErrorIsPropagated(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	storage.IdentityCountReturns(0, errors.New("count failed"))

	err := reg.BindIdentity(t.Context(), []byte("id"), "e", "w", nil, "conf-1")
	require.Error(t, err)
	require.Zero(t, storage.StoreIdentityCallCount(), "the limit must fail closed when it cannot be evaluated")
}

// Concurrent bindings must not oversubscribe the wallet.
func TestBindIdentity_ConcurrentBindingsRespectLimit(t *testing.T) {
	reg, storage, _, _ := newRegistryWithFakes()
	countingStorage(storage, role.MaxIdentitiesPerWallet-3)
	ctx := t.Context()
	...
	for range 20 {
		wg.Go(func() {
			if err := reg.BindIdentity(ctx, []byte("id"), "e", "w", nil, "conf-1"); err == nil {
				accepted.Add(1)
			}
		})
	}
	...
	require.Equal(t, int64(3), accepted.Load(), "exactly the remaining slots must be handed out")
	require.Equal(t, 3, storage.StoreIdentityCallCount())
}
