...
func TestProviderObservesAreMe(t *testing.T) {
	t.Run("a single identity is attributed to itself", func(t *testing.T) {
		...
		areMe, err := o.provider.AreMe(t.Context(), id)
		require.NoError(t, err)
		assert.Len(t, areMe, 1)
	...
	t.Run("a batch is left unattributed", func(t *testing.T) {
		...
		_, err := o.provider.AreMe(t.Context(), driver.Identity("first"), driver.Identity("second"))
		require.NoError(t, err)
...
// TestProviderReportsAreMeStorageFailure covers the contract of a failed lookup: the result is
// nil rather than a cache-only partial (a caller must not mistake "couldn't check" for "not
// mine"), and the failure is reported both through the error and to the observer.
func TestProviderReportsAreMeStorageFailure(t *testing.T) {
	...
	result, err := o.provider.AreMe(t.Context(), cached, driver.Identity("unknown_identity"))
	require.Error(t, err)
	assert.Nil(t, result, "the cache-only partial is deliberately not returned on failure")
...
func TestProviderIsMeReportsOneEvent(t *testing.T) {
	...
	isMe, err := o.provider.IsMe(t.Context(), id)
	require.NoError(t, err)
	assert.True(t, isMe)
