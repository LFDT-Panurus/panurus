...
func TestConfigDefaultsRejectsInvalidValues(t *testing.T) {
	tests := []struct {
		...
		{
			// A positive window smaller than the number of slots divides to a zero slot
			// duration, which would freeze the sliding window forever.
			name:   "window too small to divide into slots",
			config: throttle.Config{Window: 5 * time.Nanosecond},
