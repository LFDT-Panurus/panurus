...
const (
	...
	// DefaultMaxPrincipals is the upper bound on the number of per-principal entries the
	// Escalator will hold simultaneously. When this limit is reached and a new, unseen
	// principal arrives, an entry is evicted to make room: the LevelNormal entry with the
	// oldest lastSeen, or, when no normal entry exists, the oldest LevelSoft entry. LevelBlocked
	// entries are never evicted, since that would silently restore a blocked principal's full
	// quota. Only when every entry is blocked is the new principal admitted without creating
	// state, so it is treated as unthrottled — a conservative choice. Soft entries are made
	// evictable so that saturating the map with cheap soft entries cannot detach, and so stop
	// escalating, every newly seen principal.
	// At the default rate (200/s) and burst (400) an attacker needs 401 requests to pin
	// one entry; 10 000 entries therefore cost ~4 million requests and roughly 28 minutes of
	// sustained traffic before the cap is reached.
	DefaultMaxPrincipals = 10_000
)
...
type Config struct {
	...
	// MaxPrincipals is the maximum number of per-principal entries held simultaneously. When
	// the map reaches this size and a new principal arrives, the Escalator evicts the oldest
	// LevelNormal entry, or the oldest LevelSoft entry when no normal one exists, to make room;
	// LevelBlocked entries are never evicted. Zero selects DefaultMaxPrincipals; a negative
	// value disables the cap (not recommended in production).
	MaxPrincipals int `yaml:"maxPrincipals,omitempty"`
}
...
func (c *Config) Defaults() error {
	...
	// The window is divided into windowSlots sub-intervals; advanceWindow computes their
	// duration as Window/windowSlots using integer (Duration) division. A window smaller than
	// windowSlots nanoseconds truncates that to zero, which freezes the sliding window: slots
	// are never cleared, per-principal totals accumulate for the whole process lifetime, and
	// every principal eventually escalates on a single error. Reject it rather than silently
	// honour a policy that cannot slide.
	if c.Window < windowSlots {
		return errors.Errorf("invalid window [%s], expected at least %s so it divides into %d non-empty slots", c.Window, time.Duration(windowSlots), windowSlots)
	}
