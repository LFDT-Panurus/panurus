...
func TestSelectorTimeout(t *testing.T) {
	...
	makeFetcher := func() *mocks.FakeTokenFetcher {
		mockFetcher := &mocks.FakeTokenFetcher{}
		mockFetcher.UnspentTokensIteratorByStub = func(ctx context.Context, _ string, _ token2.Type) (sherdlock.Iterator[*token2.UnspentTokenInWallet], error) {
			it := &mocks.FakeIterator[*token2.UnspentTokenInWallet]{}
...
func TestSelectorResourceLimits(t *testing.T) {
	...
	makeBatchFetcher := func(batchSize int) *mocks.FakeTokenFetcher {
		...
		f.UnspentTokensIteratorByStub = func(_ context.Context, _ string, _ token2.Type) (sherdlock.Iterator[*token2.UnspentTokenInWallet], error) {
			it := &mocks.FakeIterator[*token2.UnspentTokenInWallet]{}
