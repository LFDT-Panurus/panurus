...
type Selector struct {
	...
	selectMu  sync.Mutex // held for the whole selection; serialises concurrent selects on one selector and lets Close drain in-flight selects
...
func (s *Selector) selectInternal(ctx context.Context, owner token.OwnerFilter, q string, tokenType token2.Type) ([]*token2.ID, token2.Quantity, int, int, int, error) {
	// Hold selectMu exclusively for the duration of the iteration. It serves two
	// purposes, and both require the write lock rather than the read lock:
	//   1. Close() (which also takes selectMu) waits for in-flight iterations
	//      instead of calling cache.Close() while cache.Next() is reading from
	//      sql.Rows.
	//   2. Manager.NewSelector returns the *same* *Selector from its selectorCache
	//      for a given txID, so two concurrent Select calls on one transaction
	//      would otherwise drive the same sql.Rows and race on swapCache — one of
	//      them closing the iterator the other still holds. A read lock only
	//      serialised selection against Close(), not against another selection;
	//      the write lock serialises selections on one selector against each
	//      other as well.
	//
	...
	for {
		if t, err := cache.Next(); err != nil {
			...
			newCache, fetchErr := s.fetcher.UnspentTokensIteratorBy(ctx, owner.ID(), tokenType)
			if fetchErr != nil {
			...
			// A non-positive limit means "unlimited": like withSelectionTimeout,
			// an omitted zero value has to stay harmless, otherwise the very
			// first token (tokensIterated == 1 > 0) would abort every selection
			// and a Config built without these limits would be unusable.
			tokensIterated++
			if s.maxTokensPerSelection > 0 && tokensIterated > s.maxTokensPerSelection {
				return nil, nil, immediateRetries, tokensIterated, lockAttempts, errors.Wrapf(
			...
			// Check lock attempt limit (non-positive means "unlimited", as above)
			lockAttempts++
			if s.maxLockAttempts > 0 && lockAttempts > s.maxLockAttempts {
				return nil, nil, immediateRetries, tokensIterated, lockAttempts, errors.Wrapf(
					token.SelectorResourceLimitExceeded,
