...
// cleanup leadership.
//
// Because a selector is closed per selection (a transaction with several
// transfer actions opens and closes one selector per action), dropping the
// counter here also resets the budget between actions: maxLocksPerTx therefore
// bounds a single selection, not the whole multi-action transaction. This is
// intentional — the counter's job is to bound one selection's lock-store growth
// — but it means the limit is a per-selection ceiling despite its name. See the
// "Scope of the counter" note in docs/security/selector_resource_limits.md.
...
func (b *boundedLocker) counter(txID transaction.ID) *txLockCounter {
	// Stamp lastLock at construction, before Lock's CAS reservation loop and its
	// later touch(). A zero lastLock reads as stale (staleAt compares against
	// UnixNano), so a counter left at zero could be evicted by a concurrent
	// EvictStaleTxState in the window between the reservation and the first
	// touch — handing the transaction a fresh budget on top of the locks it just
	// took. Being born "fresh" instead closes that race.
	fresh := &txLockCounter{}
	fresh.touch(time.Now())
	v, _ := b.counts.LoadOrStore(txID, fresh)
