...
// Config holds the token selector settings read from the token.selector key,
// covering the driver choice, retry and lease timing, fetcher cache tuning,
// per-wallet rate limiting, and the resource Limits that guard against
// algorithmic attacks.
type Config struct {
...
func (c *Config) GetLimits() Limits {
	...
	if limits.MaxLocksPerTransaction <= 0 {
		// Clamp the default to the resolved MaxTokensPerSelection. The documented
		// way to tighten security (docs/security/selector_resource_limits.md) is to
		// lower maxTokensPerSelection alone; if maxLocksPerTransaction then resolved
		// to its own (larger) default, Validate()'s
		// maxLocksPerTransaction <= maxTokensPerSelection invariant would fail and
		// the node would refuse to start. Defaulting to the smaller of the two keeps
		// that configuration valid while never exceeding the operator's iteration cap.
		limits.MaxLocksPerTransaction = min(defaultMaxLocksPerTransaction, limits.MaxTokensPerSelection)
	}
	if limits.SelectionTimeout <= 0 {
		...
		// retries. With the defaults (3 retries, 5s interval) that reaches
		// ~15s of backoff, so a fixed 30s ceiling would turn ordinary
...
func (c *Config) Validate() error {
	...
	// the runtime will actually apply. Testing the resolved limits rather than
	// the raw c.Limits.* fields is what catches a partially specified config:
	// e.g. maxLockAttempts=100 with maxTokensPerSelection unset resolves to
	// maxTokensPerSelection=10000, an impossible combination that would abort
	// every selection partway through, so it must be rejected here.
	limits := c.GetLimits()
