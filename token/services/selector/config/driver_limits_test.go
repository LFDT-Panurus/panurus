...
func TestConfig_Validate(t *testing.T) {
	...
	t.Run("invalid: partial config, maxLockAttempts set but maxTokensPerSelection left at default", func(t *testing.T) {
		// Only maxLockAttempts is specified; maxTokensPerSelection resolves to
		// its default (10000). The effective combination (100 < 10000) aborts
		// every selection partway through, so Validate must reject it even though
		// the raw c.Limits.MaxTokensPerSelection field is zero.
		cfg := &Config{
			Limits: Limits{
				MaxLockAttempts: 100,
			},
	...
	t.Run("invalid: partial config, maxLocksPerTransaction set but maxTokensPerSelection left at default", func(t *testing.T) {
		// maxLocksPerTransaction=20000 against the default maxTokensPerSelection
		// (10000) is impossible; the unset field must not skip the check.
		cfg := &Config{
			Limits: Limits{
				MaxLocksPerTransaction: 20000,
			},
		}
		err := cfg.Validate()
		require.Error(t, err)
...
func TestConfig_DefaultValues(t *testing.T) {
	t.Run("default values are reasonable", func(t *testing.T) {
		...
		assert.Equal(t, 3, defaultMaxRetries, "default max retries should be 3")
