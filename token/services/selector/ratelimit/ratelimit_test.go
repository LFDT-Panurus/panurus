...
func TestRateLimiter_Allow(t *testing.T) {
	t.Run("allows requests within rate limit", func(t *testing.T) {
		rl := NewTokenBucketRateLimiter(10, 10, 0, 0) // 10 req/sec, burst of 10
		identity := "wallet1"
	...
	t.Run("refills tokens over time", func(t *testing.T) {
		rl := NewTokenBucketRateLimiter(10, 10, 0, 0) // 10 req/sec
		identity := "wallet1"
	...
	t.Run("isolates different identities", func(t *testing.T) {
		rl := NewTokenBucketRateLimiter(5, 5, 0, 0)
		identity1 := "wallet1"
		identity2 := "wallet2"
	...
	t.Run("empty identity is never throttled", func(t *testing.T) {
		rl := NewTokenBucketRateLimiter(1, 1, 0, 0)
		// An empty wallet id means "no policy": always allowed, and never
		// consumes a bucket.
		for range 10 {
			require.NoError(t, rl.Allow(""))
		}
		_, exists := rl.GetStats("")
		assert.False(t, exists, "empty identity must not create a bucket")
	})

	t.Run("handles concurrent requests", func(t *testing.T) {
		rl := NewTokenBucketRateLimiter(100, 100, 0, 0)
		identity := "wallet1"
		numGoroutines := 10
...
func TestRateLimiter_Reset(t *testing.T) {
	...
	t.Run("resets all identities", func(t *testing.T) {
		...
		require.ErrorIs(t, rl.Allow(identity1), token.SelectorRateLimited)
		require.ErrorIs(t, rl.Allow(identity2), token.SelectorRateLimited)
...
func TestRateLimiter_GetStats(t *testing.T) {
	t.Run("returns stats for existing identity", func(t *testing.T) {
		rl := NewTokenBucketRateLimiter(10, 10, 0, 0)
		identity := "wallet1"
...
func TestRateLimiter_Parallel(t *testing.T) {
	...
	t.Run("single identity: allowed count equals burst size", func(t *testing.T) {
		...
		const burst = 50
		rl := NewTokenBucketRateLimiter(float64(burst), float64(burst), 0, 0)
		defer rl.Stop()
	...
	t.Run("parallel eviction and Allow do not race", func(t *testing.T) {
		...
		rl := NewTokenBucketRateLimiter(1000, 1000, ttl, 10*time.Millisecond)
		defer rl.Stop()
...
func TestNewRateLimiter_BurstValidation(t *testing.T) {
	t.Run("adjusts burst to match rate if too small", func(t *testing.T) {
		rl := NewTokenBucketRateLimiter(10, 5, 0, 0) // burst < rate
	...
	t.Run("keeps burst if larger than rate", func(t *testing.T) {
		rl := NewTokenBucketRateLimiter(10, 20, 0, 0)
...
func TestRateLimiter_IdleEviction(t *testing.T) {
	t.Run("evicts idle bucket after TTL", func(t *testing.T) {
		...
		rl := NewTokenBucketRateLimiter(10, 10, ttl, 20*time.Millisecond)
		defer rl.Stop()
	...
	t.Run("keeps active bucket alive", func(t *testing.T) {
		ttl := 100 * time.Millisecond
		rl := NewTokenBucketRateLimiter(100, 100, ttl, 20*time.Millisecond)
		defer rl.Stop()
	...
	t.Run("Stop waits for goroutine", func(t *testing.T) {
		rl := NewTokenBucketRateLimiter(10, 10, 500*time.Millisecond, 100*time.Millisecond)
