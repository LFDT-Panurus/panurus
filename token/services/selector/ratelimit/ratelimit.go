...
// Package ratelimit provides a selector-agnostic, per-identity rate limiter used
// as the Token SDK's built-in default throttling for token selection.
//
// The limiter is shared by both selector drivers (simple and sherdlock): each
// driver wraps its Locker with a small adapter that calls Limiter.Allow with the
// wallet id the tokens are being selected for. When Allow denies a request it
// returns an error wrapping token.SelectorRateLimited, which both selectors detect
// (via errors.Is) and treat as a hard, fail-fast stop.
//
// Applications that need a different policy (e.g. a distributed/Redis-backed
// limiter shared across processes, or a quota) can either supply their own
// Limiter implementation or replace the Locker entirely.
package ratelimit
...
// Limiter throttles requests per identity (wallet id). Applications may supply
// their own implementation to replace the built-in token-bucket limiter.
type Limiter interface {
	// Allow returns nil if a request from the given identity may proceed, or a
	// non-nil error (wrapping token.SelectorRateLimited) if the request is throttled.
...
// TokenBucketRateLimiter is the built-in Limiter: it manages a token bucket per
// identity. Idle buckets (those not accessed for longer than idleTTL) are evicted
...
//	burstSize:         maximum burst of requests allowed (coerced up to requestsPerSecond).
...
func NewTokenBucketRateLimiter(requestsPerSecond float64, burstSize float64, idleTTL time.Duration, cleanupInterval time.Duration) *TokenBucketRateLimiter {
...
// Returns nil if allowed, or an error wrapping token.SelectorRateLimited otherwise.
// An empty identity is always allowed (no throttling), matching the selector
// contract where an empty wallet id means "no policy".
