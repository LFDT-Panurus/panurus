...
func TestSelector_ResourceTracking(t *testing.T) {
	// The per-selection tallies live on the stack of a single Select call
	// (selectionCounters), not on *selector, so two selections on the same
	// selector cannot leak counts into one another. This test pins that
	// independence: identical back-to-back selections must produce identical
	// results with no residual state carried over from the first.
	t.Run("selections are independent, no leaked state", func(t *testing.T) {
		tokens := []*token2.UnspentToken{
		...
		// First selection
		firstIDs, firstSum, err := s.Select(ctx, filter, "50", "USD")
		require.NoError(t, err)

		// Second selection on the same selector must be unaffected by the first.
		secondIDs, secondSum, err := s.Select(ctx, filter, "50", "USD")
		require.NoError(t, err)

		assert.Equal(t, firstIDs, secondIDs, "identical selections should select the same tokens")
		assert.Equal(t, firstSum.Decimal(), secondSum.Decimal(), "identical selections should select the same quantity")
	})
}
