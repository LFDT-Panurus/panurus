...
func NewService(lockerProvider LockerProvider, c ConfigProvider, opts ...ratelimit.Option) *SelectorService {
	...
	if err := cfg.Validate(); err != nil {
		...
		// wholesale would silently drop retryInterval and every rateLimit* field
		// (the limiter is built from cfg below), so one mistyped limit could turn
		// off per-wallet rate limiting.
