...
func TestLocker_QuotaEnforcement(t *testing.T) {
	t.Run("enforces per-identity quota", func(t *testing.T) {
		...
		for i := range 5 {
			...
			_, err := locker.LockWithIdentity(ctx, identity, tokenID, txID, identity, false)
	...
	t.Run("quota is per-identity", func(t *testing.T) {
		...
		for i := range 3 {
			...
			_, err := locker.LockWithIdentity(ctx, identity1, tokenID, "tx1", identity1, false)
		...
		for i := range 3 {
			...
			_, err := locker.LockWithIdentity(ctx, identity2, tokenID, "tx2", identity2, false)
	...
	t.Run("quota decreases on unlock", func(t *testing.T) {
		...
		for i := range 3 {
			...
			_, err := locker.LockWithIdentity(ctx, identity, tokenID, "tx1", identity, false)
			require.NoError(t, err)
		}
		...
		// Unlock 2 tokens
		locker.UnlockIDs(ctx, identity, tokenIDs[0], tokenIDs[1])
	...
	t.Run("quota decreases on unlock by txID", func(t *testing.T) {
		...
		for i := range 3 {
			tokenID := &token2.ID{TxId: "token2", Index: uint64(i)}
			_, err := locker.LockWithIdentity(ctx, identity, tokenID, "tx2", identity, false)
...
func TestLocker_QuotaAndRateLimitCombined(t *testing.T) {
	...
	t.Run("backward compatibility with Lock method", func(t *testing.T) {
		...
		// Lock method with empty owner should work without identity tracking
		for i := range 20 {
			tokenID := &token2.ID{TxId: "token", Index: uint64(i)}
