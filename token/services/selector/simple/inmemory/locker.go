...
func (d *locker) lockInShard(ctx context.Context, s *shard, owner string, id *token2.ID, txID string, reclaim bool) (string, error) {
	...
	// Check lock count limit (if configured). The counter is keyed by txID
	// *within this owner's shard*, so it bounds the locks a single selection
	// takes for one owner. It is not a global per-transaction cap: a transaction
	// that spends from several wallets is counted separately in each owner shard
	// and so may hold up to maxLocksPerTx per shard. See the "Scope of the
	// counter" note in docs/security/selector_resource_limits.md.
	...
	if ok {
		...
		if reclaim {
			...
			if reclaimed {
				// deleteLocked, not a raw delete, so the previous holder's
				// txLocks count is decremented: the entry is re-created below
				// under txID, and skipping the decrement would leak a dead
				// count for observedTxID on a shard that never empties.
				s.deleteLocked(k)
			}
			if !reclaimed {
