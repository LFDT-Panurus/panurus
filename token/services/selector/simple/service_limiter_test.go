...
// These tests cover which limiter NewService installs. The simple driver reaches selection
// throttling through the same ratelimit decorator as sherdlock, whose enforcement is
// covered by the ratelimit package's tests.
...
// requires the next one to be throttled.
...
// Nothing is throttled unless it is asked for.
...
// The simple driver must honour the same switch as sherdlock: before this existed, the
// config knob was read by one driver and ignored by the other.
...
func TestShutdown_WithoutLimiter(t *testing.T) {
	svc := NewService(nil, &stubConfigProvider{}, WithLimiter(nil))
...
func TestShutdown_DoesNotStopAnApplicationSuppliedLimiter(t *testing.T) {
	spy := &spyLimiter{}
	svc := NewService(nil, &stubConfigProvider{}, WithLimiter(spy))
...
func TestShutdown_StopsALimiterItCreated(t *testing.T) {
	svc := NewService(nil, &stubConfigProvider{apply: func(c *config.Config) {
		c.RateLimitEnabled = true
	}})
	require.NotNil(t, svc.limiter)
