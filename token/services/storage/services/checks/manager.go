...
func (m *Manager) checksLoop() {
	...
	// Created only after the initial sweep completes, so the first tick is a full
	// ScanInterval away from here rather than from before a sweep that can itself
	// run for close to ScanInterval (Timeout is clamped to it). Creating the ticker
	// earlier could leave a tick already buffered by the time the loop below reaches
	// it, firing the next sweep back to back with no real gap.
...
func (m *Manager) runSweep(ctx context.Context) error {
	...
	if !acquired {
		...
		// A non-leader replica does not sweep, so it must not keep exporting whatever
		// FindingsOpen it last reported as leader: that value goes stale the moment
		// leadership moves elsewhere, and the docs call this gauge the one to alert on.
		m.resetFindingsOpenGauge()
...
// allSeverities lists every severity the FindingsOpen gauge reports on, so that
// a severity with no open findings is still Set to 0 instead of left absent.
var allSeverities = []dbcommon.Severity{dbcommon.SeverityInfo, dbcommon.SeverityWarning, dbcommon.SeverityCritical}

// resetFindingsOpenGauge zeroes FindingsOpen for every severity. Called on the
// non-leader path, where this replica reports no findings of its own and must
// not keep exporting a stale value from the last sweep it led.
func (m *Manager) resetFindingsOpenGauge() {
	for _, severity := range allSeverities {
		m.metrics.FindingsOpen.With("role", m.role, "severity", severity.String()).Set(0)
	}
}

// report logs and counts what a sweep found.
func (m *Manager) report(findings []dbcommon.Finding, resolved int64) {
	open := make(map[dbcommon.Severity]int, len(allSeverities))
	for _, severity := range allSeverities {
		open[severity] = 0
	}
