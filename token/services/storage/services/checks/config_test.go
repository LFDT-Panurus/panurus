...
func TestLoadConfig_ExplicitTimeoutExceedingScanIntervalIsClamped(t *testing.T) {
	// An explicit timeout that exceeds scanInterval fails validateConfig, which
	// takes down the whole owner/auditor service at startup. LoadConfig clamps
	// it instead of leaving a checks-only misconfiguration able to disable
	// token transaction processing for the node.
	cfg := loadConfig(t, `
token:
  tms:
    n1c1ns1:
