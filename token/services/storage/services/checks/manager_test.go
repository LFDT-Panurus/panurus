...
// A replica that loses leadership between sweeps must not keep exporting the
// FindingsOpen values it reported the last time it was leader: that gauge is
// documented as the one to alert on, and a stale non-zero value looks like an
// unresolved problem long after this replica stopped being the one checking.
func TestManager_RunSweep_NotLeader_ResetsFindingsOpenGauge(t *testing.T) {
	storage := &mock.Storage{}
	checker := &mock.Checker{}
	provider := newFakeMetricsProvider()
	storage.AcquireLeadershipReturns(nil, false, nil)
	manager := checks.NewManager(logging.MustGetLogger(), storage, checker, checks.NewMetrics(provider), testConfig(), checks.RoleOwner, testTMSID)

	require.NoError(t, manager.RunSweep(t.Context()))

	for _, severity := range []string{"info", "warning", "critical"} {
		assert.Equal(t, []float64{0}, provider.gaugeSetValues("storage_checks_findings_open", "role", checks.RoleOwner, "severity", severity))
	}
}
