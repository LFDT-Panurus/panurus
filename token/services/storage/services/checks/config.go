...
func LoadConfig(cfg *config.Configuration) (Config, error) {
	...
	if result.Timeout > result.ScanInterval {
		// A timeout left outliving its scan interval fails validateConfig, which
		// takes down the whole owner/auditor service at startup (Start returns the
		// error straight to the lazy service provider). That can happen whether the
		// config never touched timeout at all (the 30m default outliving a shorter
		// scanInterval-only override) or explicitly set both to inconsistent
		// values, so clamp either case instead of rejecting a config that is
		// otherwise fine.
