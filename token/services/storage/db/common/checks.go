...
// "expected to resolve on its own" (a ledger that has not caught up with a
// restarted node's own committed history yet, for example), which this contract
// must not turn into a false alarm on every legacy caller.
//
// This is not a blanket "Info never matters" policy: a check that wants a
// specific inconclusive result to still reach the legacy caller (see
// compareTransactionStatus's Confirmed case below) reports it above Info in the
// first place, precisely so it survives this filter. The structured findings
// table this feeds in parallel keeps every finding regardless, since severity is
// representable there and losing any of it would hide real
// degraded-observability signal from anyone looking at it directly.
...
func (a *DefaultCheckers) walkTransactions(ctx context.Context, tmsID token.TMSID, params driver.QueryTransactionsParams, fn func(*driver.TransactionRecord) error) error {
	...
	// QueryTransactions orders by (stored_at, tx_id), so every movement row of a
	// given transaction is contiguous across the whole walk, including across a
	// page boundary. That makes "is this the same transaction as the row right
	// before it" enough to dedupe, without tracking every distinct tx_id seen
	// so far: a set that would otherwise grow with total ledger history when
	// TransactionWindow is unset.
	lastTxID := ""
	first := true
	for {
		count, err := func() (int, error) {
			...
			for {
				...
				if !first && transactionRecord.TxID == lastTxID {
					continue
				}
				first = false
				lastTxID = transactionRecord.TxID
...
func (a *DefaultCheckers) compareTransactionStatus(l *network.Ledger, transactionRecord *driver.TransactionRecord) []Finding {
	...
	if err != nil {
		...
		//
		// Severity mirrors what the pre-Finding checker used to surface on-demand:
		// a Confirmed record is the one case that always produced a message there
		// (the node believes it holds spendable tokens it cannot verify the ledger
		// ever accepted, which is worth a caller's attention even mid-uncertainty),
		// so only that case is raised above Info to reach the legacy on-demand
		// contract (see AsNamedChecker). Every other local status stays Info, since
		// the old code was silent for them too - most commonly hit right after a
		// node restart, before a just-committed transaction's ledger status is
		// queryable again, which is exactly the self-resolving case Info exists for.
		severity := SeverityInfo
		if transactionRecord.Status == driver.Confirmed {
			severity = SeverityWarning
		}
...
func (a *DefaultCheckers) checkUnspentBatch(
	...
	if err != nil {
		// every backend reports an absent token as a nil entry at its position rather
		// than an error, so a batch-level error here means the call itself could not
		// be completed (e.g. the ledger is unreachable or the request was malformed),
		// not that a token is missing. Resolving the whole batch as a single
		// check_failed would lose every other token's comparison over that one
		// failure, so this falls back to resolving the batch one token at a time.
	...
	for i, id := range ids {
		if i >= len(ledgerContent) || ledgerContent[i] == nil {
			// both backends preserve the batch length and report an absent token as a
...
func (a *DefaultCheckers) checkUnspentOneByOne(
	...
	for _, id := range ids {
		...
		if len(ledgerContent) != 1 || ledgerContent[0] == nil {
			// both backends report an absent token as a nil entry rather than a
			// shorter slice or an error. See the identical reasoning in
			// checkUnspentBatch.
			findings = append(findings, Finding{
				Checker:  CheckerUnspentTokens,
				Code:     CodeTokenMissingOnLedger,
