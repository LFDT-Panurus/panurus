...
func TAmountValidation(t *testing.T, db TestTokenDB) {
	...
	// A negative amount cannot come from token.ToQuantity, but a record built by hand could
	// carry one: the magnitude alone would pass the range check, so the sign is refused too.
	negative := bigAmountRecord()
	negative.TxID = "txNegative"
	negative.Amount = big.NewInt(-1)
	require.ErrorIs(t, db.StoreToken(ctx, negative, []string{"alice"}), driver2.ErrAmountNegative)
...
// createAndDeleteToken stores then deletes a token, for tests that need a "deleted token" fixture.
...
func TGetDeletedTokensPendingSKICleanup(t *testing.T, db TestTokenDB) {
	...
	t.Run("EmptyDatabase", func(t *testing.T) {
		testSKICleanupEmptyDatabase(t, ctx, db)
	})
	t.Run("NoDeletedTokens", func(t *testing.T) {
		testSKICleanupNoDeletedTokens(t, ctx, db)
	})
	t.Run("BasicFunctionality", func(t *testing.T) {
		testSKICleanupBasicFunctionality(t, ctx, db)
	})
	t.Run("ExcludeCleanedTokens", func(t *testing.T) {
		testSKICleanupExcludeCleanedTokens(t, ctx, db)
	})
	t.Run("TimeFiltering", func(t *testing.T) {
		testSKICleanupTimeFiltering(t, ctx, db)
	})
	t.Run("LimitParameter", func(t *testing.T) {
		testSKICleanupLimitParameter(t, ctx, db)
	})
	t.Run("OrderingBySpentAt", func(t *testing.T) {
		testSKICleanupOrderingBySpentAt(t, ctx, db)
	})
	t.Run("MultipleOwnerTypes", func(t *testing.T) {
		testSKICleanupMultipleOwnerTypes(t, ctx, db)
	})
	t.Run("ZeroLimit", func(t *testing.T) {
		testSKICleanupZeroLimit(t, ctx, db)
	})
	t.Run("AllTokensCleaned", func(t *testing.T) {
		testSKICleanupAllTokensCleaned(t, ctx, db)
	})
	t.Run("MultipleIndices", func(t *testing.T) {
		testSKICleanupMultipleIndices(t, ctx, db)
	})
...
// testSKICleanupEmptyDatabase covers an empty database, which should return an empty slice.
func testSKICleanupEmptyDatabase(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
...
// testSKICleanupNoDeletedTokens covers a database with only active (non-deleted) tokens.
func testSKICleanupNoDeletedTokens(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
	tr := driver2.TokenRecord{
		TxID:           "active1",
		Index:          0,
...
// testSKICleanupBasicFunctionality covers deleted tokens without cleanup records.
func testSKICleanupBasicFunctionality(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
...
// testSKICleanupExcludeCleanedTokens covers exclusion of tokens that already have cleanup records.
func testSKICleanupExcludeCleanedTokens(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
...
// testSKICleanupTimeFiltering covers filtering to only tokens older than the given duration.
func testSKICleanupTimeFiltering(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
...
// testSKICleanupLimitParameter covers respecting the limit parameter.
func testSKICleanupLimitParameter(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
...
// testSKICleanupOrderingBySpentAt covers that results are ordered by spent_at ascending.
func testSKICleanupOrderingBySpentAt(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
...
// testSKICleanupMultipleOwnerTypes covers deleted tokens across multiple owner types.
func testSKICleanupMultipleOwnerTypes(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
...
// testSKICleanupZeroLimit covers a zero limit returning empty results.
func testSKICleanupZeroLimit(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
...
// testSKICleanupAllTokensCleaned covers the case where every deleted token already has a
// cleanup record.
func testSKICleanupAllTokensCleaned(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
...
// testSKICleanupMultipleIndices covers multiple indices for the same transaction.
func testSKICleanupMultipleIndices(t *testing.T, ctx context.Context, db TestTokenDB) {
	t.Helper()
