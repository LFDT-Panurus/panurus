...
// unspentTokenFields is the projection both branches of
// UnspentTokensIteratorBy select, in the exact order
// dedupedTokenRowsIterator.Next scans them. UNION / UNION ALL requires both
// branches to project the same columns in the same order, so the list lives in
// one place; keep it in sync with the Scan call in dedupedTokenRowsIterator.Next.
//
...
// DESC), and the final walletIDField is read for the dedup key only.
//
// walletIDField controls what that dedup column holds, and it differs by branch
// and path:
//   - Branch 2 (delegation) and the unlimited path always pass
//     ownership.wallet_id, so a (token, ownership) pair is the dedup unit and a
//     shared-ownership token surfaces once per owning wallet — the behaviour the
//     walletID="" listing relies on.
//   - Branch 1 on the limited path passes tokens.owner_wallet_id instead. That
//     branch filters owner_wallet_id = walletID, so the column is the constant
//     walletID regardless of how many ownership rows the token has. Its
//     ownership JOIN would otherwise emit one row per ownership row of a
//     directly-owned token (all with the same amount), and under UNION those
//     distinct ownership.wallet_id values would count as distinct rows against
//     LIMIT — surfacing a single physical token several times and pushing real
//     tokens past the limit, which the selector misreads as insufficient funds.
//     Projecting the constant owner_wallet_id collapses them to one row.
//
// Adding a column here without extending that Scan makes every row fail with
// "expected N destination arguments in Scan".
func unspentTokenFields(tokenTable common3.Table, walletIDField common3.Field) []common3.Field {
...
func buildUnspentTokensIteratorByQuery(db *TokenStore, walletID string, tokenType token.Type, limit int) (string, []any) {
	...
	// On the limited path, project the constant owner_wallet_id as the dedup
	// column so a directly-owned token with several ownership rows collapses to a
	// single UNION row and LIMIT counts distinct tokens (see unspentTokenFields).
	// This is safe only because branch 1 filters owner_wallet_id = walletID,
	// which the limited path (a selector querying one wallet) always does. On the
	// unlimited path walletID may be empty, so keep ownership.wallet_id there to
	// preserve the one-row-per-(token,wallet) listing semantics.
	branch1WalletIDField := ownershipTable.Field("wallet_id")
	if limit > 0 {
		branch1WalletIDField = tokenTable.Field("owner_wallet_id")
	}
	branch1 := q.Select().
		Fields(unspentTokenFields(tokenTable, branch1WalletIDField)...).
	...
	branch2 := q.Select().
		Fields(unspentTokenFields(tokenTable, ownershipTable.Field("wallet_id"))...).
		From(tokenTable.Join(ownershipTable, joinCond)).
