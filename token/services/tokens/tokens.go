...
func (t *Service) Parse(
	...
	for _, output := range os.Outputs() {
		tta, ok, err := parseOutput(ctx, auth, requestAnchor, output, auditorFlag, precision)
		if err != nil {
			return nil, nil, err
		}
		if ok {
...
// parseOutput processes a single output to identify its relation with the current TMS (mine,
// audited, issued) and, if it should be kept, builds the TokenToAppend for it. A redeem (empty
// owner) is kept only if it was redeemed against an issuer known to this node. ok is false when
// the output should be discarded.
func parseOutput(ctx context.Context, auth driver.Authorization, requestAnchor token.RequestAnchor, output *token.Output, auditorFlag bool, precision uint64) (TokenToAppend, bool, error) {
	...
	if len(output.Token.Owner) == 0 {
		return parseRedeemOutput(ctx, auth, requestAnchor, output, auditorFlag, precision)
	}
...
// parseRedeemOutput handles a redeem output (empty owner): it is kept only if it was redeemed
// against an issuer identity known to this node, in which case it is recorded with Flags.Redeemed
// set. ok is false when the redeem should be discarded.
func parseRedeemOutput(ctx context.Context, auth driver.Authorization, requestAnchor token.RequestAnchor, output *token.Output, auditorFlag bool, precision uint64) (TokenToAppend, bool, error) {
