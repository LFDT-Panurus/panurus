...
// envBoundKeys lists every config key a CORE_<KEY> environment variable may override.
//
// AutomaticEnv on its own is not enough. Unmarshal walks AllKeys(), which holds only
// the keys viper already knows from the config file or from an explicit binding, while
// AutomaticEnv resolves lazily on Get. A key present only in the environment is
// therefore never visited and its override silently does nothing - so CORE_DATASOURCE
// fails the "dataSource must not be empty" check, and CORE_TABLEPREFIX /
// CORE_TABLENAMEPARAMS fail silently and resolve the wrong tables. Binding each key up
// front puts it in AllKeys() so Unmarshal sees it.
//
// tableNames is deliberately absent: it is a map, which a single environment variable
// cannot express.
var envBoundKeys = []string{
	"driver", "dataSource", "tablePrefix", "skipPrefix", "tableNameParams",
}
...
func LoadConfig(path string) (Config, error) {
	...
	// The prefix must be set on this instance: AutomaticEnv on a fresh viper would
	// otherwise bind bare DATASOURCE/DRIVER, letting an unrelated variable silently
	// repoint the tool at another database, and would ignore the documented CORE_ form.
	v.SetEnvPrefix("core")
	v.SetEnvKeyReplacer(strings.NewReplacer(".", "_"))
	v.AutomaticEnv()
	for _, key := range envBoundKeys {
		if err := v.BindEnv(key); err != nil {
			return Config{}, errors.Wrapf(err, "failed to bind environment variable for key %q", key)
		}
	}
