...
// runConcurrentEURTransfers fires 5 concurrent transferWithSelector calls from bob to alice for
// random EUR amounts, waits for all of them to complete, and returns their total so the caller
// can verify bob's resulting balance.
...
// runRacingYUANTransfers has bob race two parallel transferWithSelector calls sending 7 YUAN each
// to charlie, when bob only has enough YUAN for one of them to succeed, then asserts exactly one
// succeeded and that its error mentions one of the two configured lock IDs ("pineapple"/"lemonade").
