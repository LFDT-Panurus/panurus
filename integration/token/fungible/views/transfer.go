...
func (t *TransferWithSelectorView) Call(context view.Context) (any, error) {
	...
	if len(t.TokenIDs) == 0 {
		...
		if err != nil {
			...
			case errors.HasCause(err, token2.SelectorResourceLimitExceeded):
				// A configured selector resource limit was hit (e.g. the
				// selection examined more tokens or attempted more locks than
				// the operator allows). This is an operator-configuration
				// outcome, not a bug, so it must be classified distinctly from
				// a genuine system failure and is deliberately not retried.
				assert.NoError(err, "kiwi")
			default:
