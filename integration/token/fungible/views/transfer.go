...
func (t *TransferWithSelectorView) Call(context view.Context) (any, error) {
	...
	if len(t.TokenIDs) == 0 {
		t.TokenIDs = t.selectTokenIDs(context, tx, amount, precision)
	}
...
// selectTokenIDs runs the default token-selection process when the caller didn't request
// specific token IDs: selects tokens of t.Type for the requested amount (retrying up to 5 times
// if t.Retry is set), closes the selector, then double-checks the vault agrees with what the
// selector chose.
func (t *TransferWithSelectorView) selectTokenIDs(context view.Context, tx *ttx.Transaction, amount token.Quantity, precision uint64) []*token.ID {
