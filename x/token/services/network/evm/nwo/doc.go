...
// developed as if it lived in a different repo: its integration surface ships with it. That is why
// this package depends only on evm-module types and never on the panurus integration module (the
// dependency runs the other way, integration requires this module).
//
// The layering mirrors fabricx: the backend's own repo (FSC for fabricx, this module for evm) ships
// the platform, and a consumer's NWO framework adds a thin adapter over it. The panurus adapter
...
//   - How the node itself is stood up (Besu in dev mode vs the fabric-x-evm gateway container) is the
//     concrete backend's concern, layered on top; this package fixes only the driver-owned shape both
//     backends share.
//
// # The two backends
//
//   - Besu backend: the primary/acceptance backend.
//   - fabric-x-evm gateway backend: boots the gateway node and its topology instead.
//
