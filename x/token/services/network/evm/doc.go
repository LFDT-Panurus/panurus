...
// before applying the transition. See docs/services/network-ethereum.md for the operator-facing
// walkthrough, docs/services/network-ethereum-internals.md for driver internals, and
// docs/services/network-ethereum-deployment.md for the deployment runbook.
//
// The package is organized into focused sub-packages:
//
//	abi         a minimal, dependency-free ABI codec for the driver's read-only contract calls
...
//	nwo         the test-network Backend contract shared by the Besu and fabric-x-evm gateway backends
//	contracts   the Solidity sources (EndorsementVerifier, TokenState, TokenStateFactory, Clones)
//	            and deploy scripts
//
